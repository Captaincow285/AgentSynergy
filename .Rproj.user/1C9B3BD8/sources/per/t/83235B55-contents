...
for (i in 1: length(mapDatasets)){
  ...
  #winrateRawData <- data.frame(matrix(ncol=length(agents), nrow = 0))
  ...
  for (j in 1:length(agents)){
    ...
    for (k in 1:length(agents)){
      ...
      if (rounds >= 300){
        ...
        #comboWRData[k] <- paste(toString(wins),toString(rounds))
      } else {
        comboWR[k] <- "ISS"
        #comboWRData[k] < "ISS"
      }
    ...
    # winrateRawData[j,] <- comboWRData
  }

  mapWinrates[[i]] <- soloWinrates
  row.names(comboWR.df) <- agents
  mapComboWR[[i]] <- comboWR.df
  #row.names(winrateRawData) <- agents
  #winrateRawDataList[[i]] <- winrateRawData
}
...
for (i in 1:length(mapComboWR)){
  ...
  #write.xlsx2(winrateRawDataList[[i]], paste0(path, "SynergyReport.xlsx"), row.names = TRUE, sheetName = paste(maps[i], "WRNums"), append = TRUE)
